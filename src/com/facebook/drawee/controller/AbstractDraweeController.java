...
/**
 * Abstract Drawee controller that implements common functionality regardless of
 * the backend used to fetch the image.
 *
 * All methods should be called on the main UI thread.
 *
 * @param <T>
 *            image type (e.g. Bitmap)
 * @param <INFO>
 *            image info type (can be same as T)
 */
@NotThreadSafe
public abstract class AbstractDraweeController<T, INFO> implements DraweeController, DeferredReleaser.Releasable, GestureDetector.ClickListener {

	/**
	 * This class is used to allow an optimization of not creating a
	 * ForwardingControllerListener when there is only a single controller
	 * listener.
	 */
	...
	/**
	 * Initializes this controller with the new id and caller context. This
	 * allows for reusing of the existing controller instead of instantiating a
	 * new one. This method should be called when the controller is in detached
	 * state.
	 * 
	 * @param id
	...
	public void addControllerListener(ControllerListener<? super INFO> controllerListener) {
		...
		// That means if it can accept <? super INFO>, it can very well accept
		// <INFO>.
	 ...
	 * 
	 * @param hierarchy
	 *            This must be an instance of {@link SettableDraweeHierarchy}
	 */
	@Override
	...
	private boolean shouldRetryOnTap() {
		...
		// For example, we expect click when fetch fails and tap-to-retry is
		// enabled.
	...
	protected void submitRequest() {
		...
		final DataSubscriber<T> dataSubscriber = new BaseDataSubscriber<T>() {
			...
			public void onNewResultImpl(DataSource<T> dataSource) {
				// isFinished must be obtained before image, otherwise we might
	...
	private void onNewResultInternal(String id, DataSource<T> dataSource, @Nullable T image, float progress, boolean isFinished, boolean wasImmediate) {
		// ignore late callbacks (data source that returned the new result is
		...
		try {
			...
			if (isFinished) {
				...
				// IMPORTANT: do not execute any instance-specific code after
				// this point
			} else {
	...
	private void onFailureInternal(String id, DataSource<T> dataSource, Throwable throwable, boolean isFinished) {
		// ignore late callbacks (data source that failed is not the one we
		// expected)
		if (!isExpectedDataSource(id, dataSource)) {
		...
		if (isFinished) {
			...
			// IMPORTANT: do not execute any instance-specific code after this
			// point
		} else {
	...
	private boolean isExpectedDataSource(String id, DataSource<T> dataSource) {
		// There are several situations in which an old data source might return
		// a result that we are no
		// longer interested in. To verify that the result is indeed expected,
