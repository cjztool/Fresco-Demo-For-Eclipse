...
public class RoundingParams {
	...
	public enum RoundingMethod {
		/**
		 * Draws rounded corners on top of the underlying drawable by overlaying
		 * a solid color which is specified by {@code setOverlayColor}. This
		 * option should only be used when the background beneath the underlying
		 * drawable is static and of the same solid color.
		 */
		OVERLAY_COLOR,

		/**
		 * Uses BitmapShader to draw bitmap with rounded corners. Works only
		 * with BitmapDrawables and ColorDrawables. IMPORTANT: Only the actual
		 * image and the placeholder image will get rounded. Other images (such
		 * as retry, failure, progress bar, backgrounds, overlays, etc.) won't
		 * get rounded.
		 */
		BITMAP_ONLY
	}
	 ...
	 *
	 * @param radii
	 *            float array of 8 radii in pixels. Each corner receives two
	 *            radius values [X, Y]. The corners are ordered top-left,
	 ...
	 *
	 * <p>
	 * For performance reasons the internal array is returned directly. Do not
	 * modify it directly, but use one of the exposed corner radii setters
	 * instead.
	 */
	 ...
	 * {@code RoundingMethod.COLOR_OVERLAY}.
	 *
	 * @param overlayColor
	 *            overlay color
	 */
	...
	/**
	 * Factory method that creates new RoundingParams with RoundAsCircle
	 * specified.
	 */
	...
	/**
	 * Factory method that creates new RoundingParams with the specified corners
	 * radius.
	 */
