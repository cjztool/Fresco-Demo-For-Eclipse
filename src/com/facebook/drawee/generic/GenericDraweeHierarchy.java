...
/**
 * A SettableDraweeHierarchy that displays placeholder image until the actual
 * image is set. If provided, failure image will be used in case of failure
 * (placeholder otherwise). If provided, retry image will be used in case of
 * failure when retrying is enabled. If provided, progressbar will be displayed
 * until fully loaded. Each image can be displayed with a different scale type
 * (or no scaling at all). Fading between the layers is supported.
 *
 * <p>
 ...
 *
 * <p>
 * Note: - ScaleType and Matrix transformations will be added only if specified.
 * If both are unspecified, then the branch for that image will be attached
 * directly. - It is not permitted to set both ScaleType transformation and
 * Matrix transformation for the same image. - A Matrix transformation is only
 * supported for actual image. - All branches (placeholder, failure, retry,
 * actual image, progressBar) are optional. If some branch is not specified it
 * won't be created. The exception is placeholder branch, which will, if not
 * specified, be created with a transparent drawable. - If overlays and/or
 * backgrounds are specified, they are added to the same fade drawable, and are
 * always displayed. - Instance of some drawable should be used by only one DH.
 * If more than one DH is being built with the same builder, different drawable
...
public class GenericDraweeHierarchy implements SettableDraweeHierarchy {
	...
	GenericDraweeHierarchy(GenericDraweeHierarchyBuilder builder) {
		...
		placeholderImageBranch = maybeApplyRoundingBitmapOnly(mRoundingParams, mResources, placeholderImageBranch);
		placeholderImageBranch = maybeWrapWithScaleType(placeholderImageBranch, builder.getPlaceholderImageScaleType());
		...
		actualImageBranch = maybeWrapWithScaleType(actualImageBranch, builder.getActualImageScaleType(), builder.getActualImageFocusPoint());
		actualImageBranch = maybeWrapWithMatrix(actualImageBranch, builder.getActualImageMatrix());
		...
		if (progressBarImageBranch != null) {
			progressBarImageBranch = maybeWrapWithScaleType(progressBarImageBranch, builder.getProgressBarImageScaleType());
		...
		if (retryImageBranch != null) {
			retryImageBranch = maybeWrapWithScaleType(retryImageBranch, builder.getRetryImageScaleType());
		...
		if (failureImageBranch != null) {
			failureImageBranch = maybeWrapWithScaleType(failureImageBranch, builder.getFailureImageScaleType());
		...
		if (numBackgrounds > 0) {
			...
			for (Drawable background : builder.getBackgrounds()) {
				layers[backgroundsIndex + index++] = maybeApplyRoundingBitmapOnly(mRoundingParams, mResources, background);
			}
		}
		...
		Drawable maybeRoundedDrawable = maybeWrapWithRoundedOverlayColor(mRoundingParams, mFadeDrawable);
	...
	private static Drawable maybeWrapWithScaleType(Drawable drawable, @Nullable ScaleType scaleType, @Nullable PointF focusPoint) {
		...
		if (drawable instanceof BitmapDrawable || drawable instanceof ColorDrawable) {
			...
			while (child != null && parent != child) {
				if (parent instanceof ForwardingDrawable && (child instanceof BitmapDrawable || child instanceof ColorDrawable)) {
					((ForwardingDrawable) parent).setCurrent(applyRounding(roundingParams, resources, child));
				}
				parent = child;
	...
	/**
	 * Gets the drawable at the specified index while skipping MatrixDrawable
	 * and ScaleTypeDrawable.
	 *
	 * <p>
	 * If <code>returnParent</code> is set, parent drawable will be returned
	 * instead. If MatrixDrawable or ScaleTypeDrawable is found at that index,
	 * it will be returned as a parent. Otherwise, the FadeDrawable will be
	...
	/**
	 * Returns the ScaleTypeDrawable at the specified index, or null if not
	 * found.
	 */
	 ...
	 *
	 * <p>
	 * Note: This uses {@link #getLayerDrawable} to find the parent drawable.
	 * Given drawable is then set as its child.
	 */
	 ...
	 * 
	 * @param resourceId
	 *            an identifier of an Android drawable or color resource.
	 */
	...
	private void updateOverlayColorRounding() {
		...
		if (mRoundingParams != null && mRoundingParams.getRoundingMethod() == RoundingParams.RoundingMethod.OVERLAY_COLOR) {
			// Overlay rounding requested - either update the overlay params or
			// add a new
			...
			if (topDrawableChild instanceof RoundedCornersDrawable) {
				...
				// important: remove the child before wrapping it with a new
				// parent!
			...
			// Overlay rounding no longer required so remove drawable that was
			// doing the rounding.
			RoundedCornersDrawable roundedCornersDrawable = (RoundedCornersDrawable) topDrawableChild;
			// Extract the drawable out of roundedCornersDrawable before setting
			// it to a new parent.
	...
	private void updateBitmapOnlyRounding() {
		if (mRoundingParams != null && mRoundingParams.getRoundingMethod() == RoundingParams.RoundingMethod.BITMAP_ONLY) {
			// Bitmap rounding - either update the params or wrap the current
			// drawable in a drawable
			// that will round it.
			...
			// No bitmap rounding requested - reset all layers so no rounding
			// occurs.
			for (int i = 0; i < mFadeDrawable.getNumberOfLayers(); i++) {
