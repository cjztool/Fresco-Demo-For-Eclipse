...
public class AspectRatioMeasure {
	 ...
	 *
	 * <p>
	 * Note: Measure spec is not changed if the aspect ratio is not greater than
	 * zero.
	 *
	 * <p>
	 * Measure spec of the layout dimension (width or height) specified as "0dp"
	 * is updated to match the measure spec of the other dimension adjusted by
	 * the aspect ratio. Exactly one layout dimension should be specified as
	 * "0dp".
	 *
	 * <p>
	 * Padding is taken into account so that the aspect ratio refers to the
	 * content without padding:
	 * {@code aspectRatio == (viewWidth - widthPadding) / (viewHeight - heightPadding)}
	 *
	 * <p>
	 * Updated measure spec respects the parent's constraints. I.e. measure spec
	 * is not changed if the parent has specified mode {@code EXACTLY}, and it
	 * doesn't exceed measure size if parent has specified mode {@code AT_MOST}.
	 *
	 * @param spec
	 *            in/out measure spec to be updated
	 * @param aspectRatio
	 *            desired aspect ratio
	 * @param layoutParams
	 *            view's layout params
	 * @param widthPadding
	 *            view's left + right padding
	 * @param heightPadding
	 *            view's top + bottom padding
	 */
	...
	private static boolean shouldAdjust(int layoutDimension) {
		// Note: wrap_content is supported for backwards compatibility, but
		// should not be used.
