 ...
 *
 * <p>
 * Hierarchy should be set prior to using this view. See {@code setHierarchy}.
 * Because creating a hierarchy is an expensive operation, it is recommended
 * this be done once per view, typically near creation time.
 *
 * <p>
 * In order to display an image, controller has to be set. See
 * {@code setController}.
 * 
 * <p>
 * Although ImageView is subclassed instead of subclassing View directly, this
 * class does not support ImageView's setImageXxx, setScaleType and similar
 * methods. Extending ImageView is a short term solution in order to inherit
 * some of its implementation (padding calculations, etc.). This class is likely
 * to be converted to extend View directly in the future, so avoid using
