...
/**
 * A forwarding drawable class - the goal is to forward (delegate) drawable
 * functionality to an inner drawable instance. ForwardingDrawable intercepts
 * the public (and protected) methods of {@link Drawable}, maintains local state
 * if needed.
 * <p>
 * Design note: It would have been very helpful to re-use Android library
 * classes like DrawableContainer, LevelListDrawable etc. DrawableContainer is
 * not directly subclassable, and the others don't allow changing the member
 * drawables.
 */
public abstract class ForwardingDrawable extends Drawable implements Drawable.Callback, TransformCallback, TransformAwareDrawable {
	...
	/**
	 * Matrix used to store temporary transform. Drawables should be accessed on
	 * UI thread only, and this matrix is used only as a temporary variable so
	 * it's safe to be static.
	 */
	 ...
	 * 
	 * @param drawable
	 *            drawable that this forwarding drawable will forward to
	 */
	...
	/**
	 * Sets a new drawable to be the delegate, and returns the old one (or
	 * null).
	 *
	 * <p>
	...
	/**
	 * As {@code setCurrent}, but without invalidating a drawable. Subclasses
	 * are responsible to call {@code invalidateSelf} on their own.
	 * 
	 * @param newDelegate
	 * @return the previous delegate
	 */
	...
	/**
	 * Gets the transformed bounds of this drawable. Note: bounds are not
	 * cropped (otherwise they would likely be the same as drawable's bounds).
	 * 
	 * @param outBounds
	...
	public void getTransformedBounds(RectF outBounds) {
		...
		// IMPORTANT: {@code getBounds} should be called after {@code
		// getParentTransform},
