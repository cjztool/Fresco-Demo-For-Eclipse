...
/**
 * Drawable that can scale underlying drawable based on specified
 * {@link ScalingUtils.ScaleType} options.
 * <p/>
...
public class ScaleTypeDrawable extends ForwardingDrawable {
	...
	// Last known dimensions of the underlying drawable. Used to avoid computing
	// bounds every time
	...
	/**
	 * Creates a new ScaleType drawable with given underlying drawable and scale
	 * type.
	 * 
	 * @param drawable
	 *            underlying drawable to apply scale type on
	 * @param scaleType
	...
	/**
	 * Sets the focus point. If ScaleType.FOCUS_CROP is used, focus point will
	 * attempted to be centered within a view. Each coordinate is a real number
	 * in [0,1] range, in the coordinate system where top-left corner of the
	 * image corresponds to (0, 0) and the bottom-right corner corresponds to
	 * (1, 1).
	 * 
	 * @param focusPoint
	...
	public void draw(Canvas canvas) {
		...
		if (mDrawMatrix != null) {
			...
			// mDrawMatrix == null means our bounds match and we can take fast
			// path
			super.draw(canvas);
		}
	}
	...
	/**
	 * Determines bounds for the underlying drawable and a matrix that should be
	 * applied on it. Adopted from android.widget.ImageView
	 */
	@VisibleForTesting
	void configureBounds() {
		...
		// We need to do the scaling ourselves, so have the underlying drawable
		// use its preferred size.
		underlyingDrawable.setBounds(0, 0, underlyingWidth, underlyingHeight);
		ScalingUtils.getTransform(mTempMatrix, bounds, underlyingWidth, underlyingHeight, (mFocusPoint != null) ? mFocusPoint.x : 0.5f, (mFocusPoint != null) ? mFocusPoint.y : 0.5f, mScaleType);
	...
	public void getTransform(Matrix transform) {
		...
		// IMPORTANT: {@code configureBounds} should be called after {@code
		// getParentTransform},
