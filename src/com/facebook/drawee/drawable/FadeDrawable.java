 ...
 *
 * <p>
 * Arbitrary number of layers is supported. 5 Different fade methods are
 * supported. Once the transition starts we will animate layers in or out based
 * on used fade method. fadeInLayer fades in specified layer to full opacity.
 * fadeOutLayer fades out specified layer to zero opacity. fadeOutAllLayers
 * fades out all layers to zero opacity. fadeToLayer fades in specified layer to
 * full opacity, fades out all other layers to zero opacity. fadeUpToLayer fades
 * in all layers up to specified layer to full opacity and fades out all other
 * layers to zero opacity.
 *
 */
public class FadeDrawable extends ArrayDrawable {
	...
	/**
	 * Determines whether to fade-out a layer to zero opacity (false) or to
	 * fade-in to the full opacity (true)
	 */
	@VisibleForTesting
	boolean[] mIsLayerOn;

	/**
	 * When in batch mode, drawable won't invalidate self until batch mode
	 * finishes.
	 */
	@VisibleForTesting
	...
	/**
	 * Begins the batch mode so that it doesn't invalidate self on every
	 * operation.
	 */
	 ...
	 * 
	 * @param index
	 *            the index of the layer to fade in.
	 */
	 ...
	 * 
	 * @param index
	 *            the index of the layer to fade out.
	 */
	 ...
	 * 
	 * @param index
	 *            the index of the layer to fade to
	 */
	 ...
	 * <p>
	 * Layers up to the specified layer inclusive will fade in, other layers
	 * will fade out.
	 * 
	 * @param index
	 *            the index of the layer to fade up to.
	 */
	...
	/**
	 * Updates the current alphas based on the ratio of the elapsed time and
	 * duration.
	 * 
	 * @param ratio
	...
	public void draw(Canvas canvas) {
		...
		switch (mTransitionState) {
			case TRANSITION_STARTING:
				...
				// if the duration is 0, update alphas to the target opacities
				// immediately
				ratio = (mDurationMs == 0) ? 1.0f : 0.0f;
				// if all the layers have reached their target opacity,
				// transition is done
			...
			case TRANSITION_NONE:
				// there is no transition in progress and mAlphas should be left
				// as is.
				done = true;
				break;
		}
	...
	/**
	 * Returns current time. Absolute reference is not important as only time
	 * deltas are used. Extracting this to a separate method allows better
	 * testing.
	 * 
	...
	/**
	 * Gets the transition state (STARTING, RUNNING, NONE). Useful for testing
	 * purposes.
	 * 
