...
public class ScalingUtils {
	 ...
	 * <p>
	 * Similar to {@link android.widget.ImageView.ScaleType}, but
	 * ScaleType.MATRIX is not supported. To use matrix scaling, use a
	 * {@link MatrixDrawable}. An additional scale type (FOCUS_CROP) is
	 * provided.
	 */
	public enum ScaleType {

		/**
		 * Scales width and height independently, so that the child matches the
		 * parent exactly. This may change the aspect ratio of the child.
		 */
		FIT_XY,

		/**
		 * Scales the child so that it fits entirely inside the parent. At least
		 * one dimension (width or height) will fit exactly. Aspect ratio is
		 * preserved. Child is aligned to the top-left corner of the parent.
		 */
		FIT_START,
		 ...
		 * Scales the child so that it fits entirely inside the parent. At least
		 * one dimension (width or height) will fit exactly. Aspect ratio is
		 * preserved. Child is centered within the parent's bounds.
		 */
		FIT_CENTER,
		 ...
		 * Scales the child so that it fits entirely inside the parent. At least
		 * one dimension (width or height) will fit exactly. Aspect ratio is
		 * preserved. Child is aligned to the bottom-right corner of the parent.
		 */
		FIT_END,
		...
		/**
		 * Scales the child so that it fits entirely inside the parent. Unlike
		 * FIT_CENTER, if the child is smaller, no up-scaling will be performed.
		 * Aspect ratio is preserved. Child is centered within parent's bounds.
		 */
		CENTER_INSIDE,

		/**
		 * Scales the child so that both dimensions will be greater than or
		 * equal to the corresponding dimension of the parent. At least one
		 * dimension (width or height) will fit exactly. Child is centered
		 ...
		 * Scales the child so that both dimensions will be greater than or
		 * equal to the corresponding dimension of the parent. At least one
		 * dimension (width or height) will fit exactly. The child's focus point
		 * will be centered within the parent's bounds as much as possible
		 * without leaving empty space. It is guaranteed that the focus point
		 * will be visible and centered as much as possible. If the focus point
		 * is set to (0.5f, 0.5f), result will be equivalent to CENTER_CROP.
		 */
		FOCUS_CROP;
		 ...
		 *
		 * <p>
		 * Used by GenericDraweeView styleable in
		 ...
		 *             if scale type is not recognized
		 */
