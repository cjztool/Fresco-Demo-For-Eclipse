 ...
 * <p>
 * This class allows us to get click events when we need them, but not to
 * consume them when we are temporarily not interested in them. Doing
 * {@code View.setClickable(true)} will cause for the view always to consume
 * click event, even if {@code View.performClick} is overridden to return false.
 * That means even though our view didn't handle the click event, the event will
 * not get propagated upwards. Result of {@code View.onTouchEvent} is handled
 * correctly though so we use that instead.
 * <p>
...
public class GestureDetector {
	 ...
	 * Resets component.
	 * <p>
	 * This will drop any gesture recognition that might currently be in
	 * progress.
	 */
	public void reset() {
